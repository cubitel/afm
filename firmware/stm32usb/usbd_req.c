...
static void USBD_GetCompatibleId(USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ *req);
...
USBD_Status  USBD_StdDevReq (USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ  *req)
{
  ...
  case USB_REQ_GET_COMPATIBLE_ID:
	USBD_GetCompatibleId(pdev, req);
	break;
...
void USBD_GetString(char *desc, uint8_t *unicode, uint16_t *len)
{
  ...
  if (desc != NULL) 
  {
    *len =  USBD_GetLen((uint8_t *)desc) * 2 + 2;
    unicode[idx++] = *len;
...
static uint8_t CompatibleId[40] = {
